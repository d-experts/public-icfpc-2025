...
func (r *Rooms) changeLabel(room, label int) {
	...
	// Ensure labels array has enough capacity
	for len(r.labels[pos]) <= idx {
		r.labels[pos] = append(r.labels[pos], -1)
	}
	r.labels[pos][idx] = label
	
	// Remove duplicates (keep only unique labels)
	unique := []int{}
	seen := make(map[int]bool)
	for _, l := range r.labels[pos] {
		if l != -1 && !seen[l] {
			unique = append(unique, l)
			seen[l] = true
		}
	}
	if len(unique) < len(r.labels[pos]) {
		r.labels[pos] = unique
	}
}
...
func (r *Rooms) connectRoom(from_room, from_door, to_room int, history *[]domain.Location) bool {
	...
	from_room3 := (from_room + 2*r.posN()) % len(r.rooms)
	to_room3 := (to_room + 2*r.posN()) % len(r.rooms)
	if !connect(from_room3, from_door, to_room3, to_door) {
		return false
	}
	if !connect(to_room3, to_door, from_room3, from_door) {
		return false
	}
	return true
}
...
func (r *Rooms) dfs(depth int, from_room int) bool {
	...
	if to_room_candidate != -1 {
		...
		to_rooms = []int{to_pos, to_pos + r.posN(), to_pos + 2*r.posN()}
	}
