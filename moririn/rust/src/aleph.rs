...
pub fn run_go_with_json(plan: &str, results: &[usize], map_data: &api::Map) -> Result<bool, Box<dyn std::error::Error>> {
    use std::io::Write;
    ...
    let json_line = stdout.lines()
        .rev() // 最後の行から検索
        .find(|line| line.trim().starts_with('{') || line.trim() == "true" || line.trim() == "false")
